...
class UrbanSound8K:
  ...
  def generate_images(self):
    ...
    for i in range(len(self.data)):
      ...
      sound_file_path=path.join(self.audio_path, "fold"+str(row['fold']), row['slice_file_name'])
      jpeg_path=path.join(self.images_path, "fold"+str(row['fold']), row['slice_file_name'])+'.jpg'
